...
// The block starts with `{/* Smart Filter Bar */}` and ends before `<div ref={parentRef}`
const filterBarRegex = /\{\/\* Smart Filter Bar \*\/\}\s*<div className="w-full relative group mb-6">[\s\S]*?\{\/\* Clear Filters Action \*\/\}\s*\{hasActiveFilters && \([\s\S]*?<\/Button>\s*\)\}\s*<\/div>\s*<\/div>\s*<\/div>/;

const filterBarMatch = pageCode.match(filterBarRegex);

if (!filterBarMatch) {
    console.error("Could not find Filter Bar block");
    process.exit(1);
}

const filterBarReplacement = `<TransactionFilters 
...
pageCode = pageCode.replace(filterBarRegex, filterBarReplacement);

// Add import
const importTarget = `const EditTransactionDialog = dynamic(() => import('./EditTransactionDialog'), { ssr: false });`;
const importReplacement = `const EditTransactionDialog = dynamic(() => import('./EditTransactionDialog'), { ssr: false });\nimport TransactionFilters from "./TransactionFilters";`;

if (!pageCode.includes('import TransactionFilters')) {
    pageCode = pageCode.replace(importTarget, importReplacement);
}

fs.writeFileSync(pagePath, pageCode);

// Create TransactionFilters.tsx
const componentCode = `import { Input } from "@/components/ui/input";
...
fs.writeFileSync('frontend/app/(dashboard)/transactions/TransactionFilters.tsx', componentCode);
console.log("Extracted TransactionFilters!");
